...
class Wizard(Toplevel): #clean up
	def __init__(self,parent,wiztitle,ICONPATH='',suffix='jpg',SEP=os.path.sep):
		self.smallfont = "Verdana 8" #adapt for different OS
		...
		# defaults
		#self.odir=os.getcwd() #this may change, we need to keep original value!!!
		self.odir=PROGDIR
		self.initVars()
	...
	def initUI(self):
		...
		self.cImage=[self.odir+'/RES/wiz1.png',self.odir+'/RES/wiz2.png',self.odir+'/RES/wiz3.png',self.odir+'/RES/wiz4.png',self.odir+'/RES/wiz5.png']
		...
		self.slide=Frame(self,relief=SUNKEN,bd=1,bg=self.text_col,width=self.winfo_width(),height=self.winfo_height()-30)
		...
		self.leftside=Frame(self.slide,relief=FLAT,bd=1,width=self.w,height=self.h,bg=self.title_button_col) #need to resize all images to 355
		...
		self.bBTN1=Button(self.bFrame,text='Cancel',command=self._BTN1,anchor=W,relief=GROOVE,bg=self.title_button_col)
		self.bBTN2=Button(self.bFrame,text='Back',command=self._BTN2,anchor=E,relief=GROOVE,bg=self.title_button_col,state=DISABLED)
		self.bBTN3=Button(self.bFrame,text='Next',command=self._BTN3,anchor=E,relief=GROOVE,bg=self.title_button_col)
	...
	def init2nd(self):
		rightside=Frame(self.slide,bd=0,relief=GROOVE,bg=self.data_col,width=self.winfo_width(),height=self.winfo_height()-30)
		...
		t.insert(END,'Please provide a valid Username that will be used throughout your session.\nValid characters are a-z äöü _ 0-9, both upper and lower case letters are accepted. In addition, you can provide up to 9 additional parameters for your session, such as Survey specific parameters like survey vessel information.\n\nRemember: iTAG is most useful when multiple people analyse the same set of images. Providing meaningful usernames can help you sort out differences once you have produced your output.')
		t.configure(state=DISABLED)
		t.pack(side=TOP,fill=X)
		
		ul=Label(userFrame,text="Username: ",relief=FLAT,justify="left",bg=self.data_col)
		self.user_e = Entry(userFrame, textvariable = self.user,width=32)
		ul.grid(row=0,column=0)
		self.user_e.grid(row=0,column=1)
		self.para_e=[]
		for i in range(0,9):
			ll=Label(userFrame,text="Parameter "+str(i+1)+": ",relief="flat",justify="left",bg=self.data_col)
			self.para_e.append(Entry(userFrame,width=32))
	...
	def init4th(self):
		rightside=Frame(self.slide,bd=0,bg=self.data_col,relief=GROOVE,width=self.winfo_width(),height=self.winfo_height()-30)
		centerFrame=Frame(rightside,relief=GROOVE,bd=2,width=460,height=self.winfo_height()-30,bg=self.data_col)
		textFrame = Frame(rightside,relief=FLAT,bd=0)
		t=Label(textFrame,text='Please provide information about the Tags you are going to use.\nYou can specify up to 9 different Tags and assign individual colours to them.\nYou can also change the names for the modifier keys.',relief=FLAT,justify='left')
		...
		for i in range(0,len(self.catnames)):
			...
			self.bCol.append(Button(catFrame, bg=self.colours_palette[i],command=lambda i=i: self.colCHOOSE(index=i),relief=SUNKEN,width=5))
		...
		for i in range(0,len(modifier)):
			...
			self.alt_key.append(Label(altFrame, text=str(modifier[i]) +": ",relief=GROOVE,justify="left",bg=self.data_col))
	...
	def init5th(self):
		...
		t.insert(END,'Congratulations, you have finished setting up your session!\nYou will find all the relevant info on your session set up below.'+wrn)
		t.configure(state=DISABLED)
		t.pack(side=LEFT,fill=BOTH,anchor='w')
		...
		for j in range(0,len(self.catnames)): #this should "wrap" labels at widget border
			ll=Label(centerFrame,text=self.catnames[j],font=self.smallfont,relief=GROOVE,justify="left",bg=self.colours_palette[j],fg=self.col_invert(self.colours_palette[j]))
			ll.grid(row=j,column=2,sticky=S+W+E)
		ll=Label(centerFrame,text="Modifiers: " + ', '.join(self.modnames),relief="flat",justify="left",bg=self.data_col)
		ll.grid(row=i+1,column=0,columnspan=2,sticky=S+W)

		optFrame=Frame(rightside,bg=self.data_col)

		Button(optFrame,text='Save settings...',command=lambda i=1:self.save_opt(i),relief=GROOVE).grid(row=0,column=0,sticky=W)

		Button(optFrame,text='Save as default settings',command=self.save_opt,relief=GROOVE).grid(row=0,column=1,sticky=W)
		optFrame.pack(side=BOTTOM,fill=X,anchor=E)
		centerFrame.pack(side=BOTTOM,fill=BOTH)
		textFrame.pack(side=TOP,fill=X,anchor=E)
		return rightside
		
	def col_invert(self,color): #ok!
	...
	def redraw(self): #redraw specific slide
		...
		if self.count == 1:
			...
			self.user_e.configure(validate = 'key', validatecommand = (self.okayUser, '%i','%S', '%P')) #$i is why %S is the new character, %P the new var

		elif self.count == 3: #file open slide
			if not self.files_good: self.bBTN3.config(state=DISABLED)
			else: self.bBTN3.config(state=NORMAL)
			...
			self.okayUser = self.register(self.check_username)
			self.catcounter.configure(validate = 'key', validatecommand = (self.change_cats, '%i','%s', '%P')) #$i is why %S is the new character, %P the new var
			for i in range (0,len(self.catnames)):
				self.cat_e[i].configure(validate = 'key', validatecommand = (self.okayCat, '%i','%S', '%P')) #$i is why %S is the new character, %P the new var
			for i in range (0,len(self.modnames)):
				self.alt_e[i].configure(validate = 'key', validatecommand = (self.okayUser, '%i','%S', '%P')) #$i is why %S is the new character, %P the new var
	...
	def openDir(self): #okish - needs support for different suffixes of jpg... doesn't allow special chars in path
		...
		if len(dirname) >0:
			...
			self.dirList = [dirname+os.sep+f for f in os.listdir(dirname) if re.search(r'.*\.(jpg|JPG|bmp|BMP)$', f)]
			if len(self.dirList)<=0:
				...
				try:
					...
					messagebox.showinfo("Error",'Please stick to the range of allowed characters in your path / filename')
					return
				self.fname = os.path.split(str(self.dirList[0]))[1]
				os.chdir(self.basename)
				path=self.basename.split(os.sep)
				m=len(path)
				if m>3:	self.shortpath='...'+os.sep+path[m-4]+os.sep+path[m-3]+os.sep+path[m-2]+os.sep+path[m-1]+os.sep
				else: self.shortpath=self.basename
				self.updateValues()
	...
	def updateValues(self):
		...
		self.out['params']=self.para #extra parameters
		...
		self.out['newdir']=self.basename + str(SEP) + self.user.get()
		self.out['tagsize']=6
		self.out['os_name']=os.name
	...
	def restore_opt(self,type=None):
		...
		if type ==None:
			out=askopenfilename(parent=self,title=CHOOSE_FILE,initialdir = self.odir,filetypes=[('Image Tag Options', '.TiO'),('all files', '.*')]) #gets covered / too large
		if out != '':
			try:
				with open(out) as f:data=pickle.load(f)
				success=True
			except:
	...
	def initVars(self):
		...
		self.para=['' for x in range(0,9)]
		self.count = 1 #slide counter
		self.colours_palette=["#FFFF00","#09FF09","#FF00FF",'#6E44E3','#C9E87B','#69E0C7','#D19B1D','#000000','#FFFFFF']
		...
		self.dirList=[]
		self.basename=''
		self.basename + str(SEP) + self.user.get()
		self.fparameter={'Directory: ':"",'File type: ':"",'File Count: ':0,'First File: ':"",'Last File: ':""}
		self.tagsize=1
		self.slidenames=[None,'Welcome to iTAG','Username & Extra Parameters','Retrieve image directory','Definition of tags','Session Summary']
	...
	def save_opt(self,type=None):
		...
		if type ==None:
			...
			out=asksaveasfilename(parent=self.parent,title='save option file as: ', initialdir=self.odir, defaultextension="TiO",filetypes=[('Image Tag options', '.TiO'),('all files', '.*')])
			messagebox.showinfo("Info",'This specific session setup has been saved at: '+out)
		try:
			with open(out,'wb') as f: pickle.dump(self.out,f)
		except:
	...
	def colCHOOSE(self,index):
		...
		if out != (None,None):
			...
			for i in self.colours_palette:
				test = test and same_cols(out[1],i)
			if not test:
				tkinter.messagebox.showwarning("Warning",'Colours may be very similar and thus difficult to distinguish')
