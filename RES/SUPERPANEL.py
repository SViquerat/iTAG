...
class MultiPanel(Toplevel): #maybe pass opened image, its faster?
	...
	def __init__(self,parent,cookie,gpsdict,ptitle,image=None):
		...
		self.Navi=NaviPanel(self.nFrame, imgpth=filename,cookie=cookie,image=image)
		self.Navi.pack()
		self.nFrame.grid(row=0,column=0)

		self.p1=Button(self.PANEL,text='File Info Panel',relief=RAISED,anchor='e', command=lambda i=1:self.toggle_vis(index=i))
		self.p1.grid(row=1,column=0,sticky=E+W)
		self.p2=Button(self.PANEL,text='Tag summary panel', relief=RAISED,anchor='e', command=lambda i=2:self.toggle_vis(index=i))
		self.p2.grid(row=3,column=0,sticky=E+W)

		self.bottoms=[None]*3

		self.infoFrame=Frame(self.PANEL)
		self.summary=InfoPanel(self.infoFrame,cookie)
	...
	def _CLOSE(self):
		self.parent.viewmenu.invoke(0) #why is this working? I don't know and I don't care. probably this is not safe, but who cares?
	...
	def update(self,cookie,i=None,j=None,index=0,gpsdict={},image=None):
		...
		if index==0: #redraw
			self.Navi.redraw(cookie,image)
			self.exifInfo.update(gpsdict)
		elif index==1: #tag add /remove
			self.summary.update(cookie,i=i,j=j)
		else:
			self.summary.update(cookie,i=i,j=j)
			self.Navi.redraw(cookie,image)
			self.exifInfo.update(gpsdict)
...
class NaviPanel(Frame): #navigation panel
	def __init__(self,parent,imgpth,cookie,image=None):
		...
		self.canvas_bg.create_rectangle((0,0,cookie['panel_canvas_size'],cookie['panel_canvas_size']),fill='red',tags=("panel_bg",0)) #in the panel
		...
		self.lSize=Label(self,text='Image size: ' + str(self.image_size_original[0])+' x '+str(self.image_size_original[1]))
	...
	def openImage(self,imgpth,cookie):
		...
		try:
			...
			draw.text((textx, texty),"You seem to have run into memory issues. Please consider either increasing your system Memory or switching to the 64bit Version of iTAG to make more Memory available. You can still use the navigational panel for navigation.",'white')
			print ("load failure")
	...
	def redraw(self,cookie,image=None):
		...
		self.lSize.configure(text='Image size: ' + str(self.image_size_original[0])+' x '+str(self.image_size_original[1]))
...
class InfoPanel(Frame): #tag counter panel
	def __init__(self,parent,cookie):
		...
		spec=cookie['catnames'] #species names
		alt=cookie['modnames'] #alt names
		catc=cookie['catcounter'] #current count
		self.counts=[[ _ for _ in range(len(alt))] for _ in range(len(spec))] #empty count labels
		self.sum=[] #species sums
		for i in range(0,len(spec)):
			label=Label(self.infoFrame,text=spec[i] + ": ",anchor='nw',relief='groove',font = LABELFONT)
		...
		for j in range (0,len(alt)): #maybe superfluous
			label=Label(self.infoFrame,text=alt[j],relief='groove',font = LABELFONT)
...
class exifPanel(Frame): #exif viewer panel
	def __init__(self,parent,gpsdict):
		...
		lat=gpsdict.get('lat')
		lon=gpsdict.get('lon')
		coord_ref=str(gpsdict.get('coord_ref'))
		altitude=str(gpsdict.get('altitude'))
		datetime=str(gpsdict.get('DateTimeOriginal'))
				
		#file info part
		MAKE=str(gpsdict.get('Make'))
		MODEL=str(gpsdict.get('Model'))
		FLENGTH=str(gpsdict.get('FocalLength'))
		EXIFV=str(gpsdict.get('ExifVersion'))

		gps_col='red'
		state='disabled'

		if gpsdict.get('Valid_GPS')=='True':
			gps_col='#2F9E3E'
			state='normal'
			lat=round(float(lat),4)
			lon=round(float(lon),4)
		file_col='red'
		if gpsdict.get('Valid_Exif')=='True':
			file_col='#2F9E3E'
		...
		self.pos=Button(self.exifFrame,text=str(lat)+', '+str(lon),relief=relief,fg=gps_col,command=lambda i=gpsdict:self.on_earth(i),state=state)
	...
	def update(self,gpsdict):
		...
		self.pos.config(text=str(lat)+', '+str(lon),fg=gps_col,state=state,command=lambda i=gpsdict:self.on_earth(i))
